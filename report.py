...
def get_or_create_folder(parent_folder_id: str, folder_name: str) -> str:
    """Lấy id thư mục con `folder_name` trong `parent_folder_id`; tạo mới nếu
    chưa có. KHÔNG xoá nội dung bên trong nếu thư mục đã tồn tại — dùng cho
    thư mục THÁNG (chứa nhiều thư mục ngày bên trong, tuyệt đối không được
    xoá đi mỗi lần chạy, khác với get_or_create_date_folder ở trên vốn xoá
    sạch nội dung cũ vì dùng cho thư mục NGÀY — nơi chỉ chứa file Excel của
    đúng ngày đó, xoá đi khi chạy lại là hợp lý)."""
    try:
        service = get_drive_service()
        if not service:
        ...
        print(f"❌ Lỗi Drive Folder (tháng): {e}")
        return parent_folder_id
...
def process_and_clean_data(
    ...
    *,
    ...
    month_folder_name = start_yesterday.strftime("%Y-%m")
    report_date_folder_name = start_yesterday.strftime("%Y-%m-%d")
    print(f"\n☁️ Chuẩn bị thư mục Drive: {month_folder_name}/{report_date_folder_name}")
    month_drive_folder_id = get_or_create_folder(DRIVE_FOLDER_ID, month_folder_name)
    target_drive_folder_id = get_or_create_date_folder(month_drive_folder_id, report_date_folder_name)
    print("-" * 60)
